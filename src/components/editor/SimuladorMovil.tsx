...
import PearlElegance from '../templates/PearlElegance'; // <-- IMPORTAMOS LA NUEVA PLANTILLA
...
export default function SimuladorMovil({ template }: Props) {
  const { estilos } = template;
  const [isOpen, setIsOpen] = useState(false);

  // Mapeo de colores incluyendo el Blanco Perla
  const colorMap: Record<string, string> = {
    ...
    Pearl: 'bg-[#fdfdfc] text-zinc-800 border-zinc-200',
  };

  const currentEnvelopeColor = colorMap[estilos.envelope.color] || 'bg-zinc-900';

  // Si cambias parámetros estéticos del sobre, lo cerramos para que veas el cambio
  useEffect(() => {
    setIsOpen(false);
  }, [estilos.envelope.color, estilos.envelope.waxSeal]);

  // ==========================================
  // DATOS MOCK (FALSOS) PARA EL SIMULADOR
  // ==========================================
  const mockInvitado = {
    nombreFamilia: 'Familia Paternina Osorio'
  };
  ...
  const mockEvento = {
    ...
    configMostrarMapaCeremonia: false,
    configMostrarMapaRecepcion: false,
    template: template // <-- Le inyectamos lo que estás editando en vivo
  };
        ...
        >
          ...
          <div className="text-center space-y-6 z-10">
            ...
            <div className="font-serif italic text-lg text-amber-100/90">
              {mockInvitado.nombreFamilia}
            </div>
          </div>
        </div>
      )}

      {/* CONTENIDO INTERIOR: LA PLANTILLA REAL EN VIVO */}
      {isOpen && (
        <div className="flex-1 overflow-y-auto scrollbar-none relative bg-[#f4f4f2]">
          
          {/* Botón flotante para cerrar el sobre en el simulador */}
          <button 
          ...
          {/* INYECTAMOS LA PLANTILLA EXACTA */}
          <PearlElegance 
            evento={mockEvento} 
            invitado={mockInvitado} 
            onOpenRsvp={() => alert('El Pop-up de confirmación se abrirá aquí para tus invitados reales.')} 
          />
...
//     Lavender: 'bg-purple-950/90 text-purple-200',
//     Burgundy: 'bg-rose-950 text-rose-200',
//     Pearl: 'bg-[#fdfdfc] text-zinc-800 border border-zinc-200', // <-- AGREGA ESTA LÍNEA
//   };
